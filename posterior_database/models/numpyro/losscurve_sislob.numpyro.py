...
def convert_inputs(inputs):
    ...
    return { 'growthmodel_id': growthmodel_id, 'n_data': n_data,
             'cohort_id': cohort_id, 't_idx': t_idx, 'n_cohort': n_cohort,
             'cohort_maxtime': cohort_maxtime, 'n_time': n_time,
             't_value': t_value, 'premium': premium, 'loss': loss }
...
def model(*, growthmodel_id, n_data, cohort_id, t_idx, n_cohort,
    ...
    def _fori__1(i, _acc__2):
        gf = _acc__2
        gf = ops_index_update(gf, ops_index[i - 1], growth_factor_weibull(
        t_value[i - 1], omega, theta) if growthmodel_id == 1 else growth_factor_loglogistic(
    ...
    def _fori__3(i, _acc__4):
        lm = _acc__4
        lm = ops_index_update(lm, ops_index[i - 1], LR[cohort_id[i - 1] - 1] * premium[
        cohort_id[i - 1] - 1] * gf[t_idx[i - 1] - 1])
        return lm
...
def generated_quantities(*, growthmodel_id, n_data, cohort_id, t_idx,
                            n_cohort, cohort_maxtime, n_time, t_value,
                            premium, loss, omega, theta, LR, mu_LR, sd_LR,
                            loss_sd):
    # Transformed parameters
    gf = empty([n_time], dtype=dtype_float)
    @jit
    ...
    def _fori__16(i, _acc__17):
        ...
        step_ratio = ops_index_update(step_ratio, ops_index[i - 1], rep_vector_int_int(
        1, n_time))
        loss_sample = ops_index_update(loss_sample, ops_index[i - 1], LR[
        i - 1] * premium[i - 1] * gf)
    ...
    def _fori__18(i, _acc__19):
        ...
        loss_prediction = ops_index_update(loss_prediction, ops_index[
        cohort_id[i - 1] - 1, t_idx[i - 1] - 1], loss[i - 1])
    ...
    def _fori__20(i, _acc__21):
        ...
        def _fori__22(j, _acc__23):
            step_ratio = _acc__23
            step_ratio = ops_index_update(step_ratio, ops_index[i - 1, j - 1], true_divide(gf[
    ...
    def _fori__24(i, _acc__25):
        ...
        def _fori__26(j, _acc__27):
            ...
            loss_prediction = ops_index_update(loss_prediction, ops_index[
            i - 1, j - 1], loss_prediction[i - 1, j - 1 - 1] * step_ratio[
            i - 1, j - 1])
            return loss_prediction
        loss_prediction = lax_fori_loop((cohort_maxtime[i - 1] + 1),
                                        n_time + 1, _fori__26,
                                        loss_prediction)
        return loss_prediction
    loss_prediction = lax_fori_loop(1, n_cohort + 1, _fori__24,
                                    loss_prediction)
    ...
    def _fori__28(i, _acc__29):
        ppc_EFC = _acc__29
        ppc_EFC = ppc_EFC + loss_prediction[i - 1, n_time - 1] - loss_prediction[
        i - 1, cohort_maxtime[i - 1] - 1]
        return ppc_EFC
    ppc_EFC = lax_fori_loop(1, n_cohort + 1, _fori__28, ppc_EFC)
    return { 'gf': gf, 'lm': lm, 'loss_sample': loss_sample,
             'step_ratio': step_ratio, 'mu_LR_exp': mu_LR_exp,
             'loss_prediction': loss_prediction, 'ppc_minLR': ppc_minLR,
             'ppc_maxLR': ppc_maxLR, 'ppc_EFC': ppc_EFC }

def map_generated_quantities(_samples, *, growthmodel_id, n_data, cohort_id,
                                          t_idx, n_cohort, cohort_maxtime,
                                          n_time, t_value, premium, loss):
    def _generated_quantities(omega, theta, LR, mu_LR, sd_LR, loss_sd):
        return generated_quantities(growthmodel_id=growthmodel_id,
                                    n_data=n_data, cohort_id=cohort_id,
                                    t_idx=t_idx, n_cohort=n_cohort,
                                    cohort_maxtime=cohort_maxtime,
                                    n_time=n_time, t_value=t_value,
                                    premium=premium, loss=loss, omega=omega,
                                    theta=theta, LR=LR, mu_LR=mu_LR,
                                    sd_LR=sd_LR, loss_sd=loss_sd)
...
def parameters_info(*, growthmodel_id, n_data, cohort_id, t_idx, n_cohort,
                       cohort_maxtime, n_time, t_value, premium, loss):
    return { 'omega': { 'shape': [] },'theta': { 'shape': [] },
             'LR': { 'shape': [n_cohort] },'mu_LR': { 'shape': [] },
             'sd_LR': { 'shape': [] },'loss_sd': { 'shape': [] }, }
