...
def transformed_data(*, N, R, culm, T, response):
    ...
    def _fori__1(j, _acc__2):
        ...
        def _fori__3(k, _acc__4):
            r = _acc__4
            r = ops_index_update(r, ops_index[k - 1, j - 1], response[
            1 - 1, k - 1])
            return r
    ...
    def _fori__11(i, _acc__12):
        ones__ = _acc__12
        ones__ = ops_index_update(ones__, ops_index[i - 1], array(1.0, dtype=dtype_float))
        return ones__
...
def model(*, N, R, culm, T, response, r, ones__):
    ...
    # Model
    observe('_alpha__13', normal(0, array(100., dtype=dtype_float)), alpha)
    observe('_theta__14', normal(0, 1), theta)
    observe('_beta__15', normal(array(0.0, dtype=dtype_float),
                                array(100., dtype=dtype_float)), beta__)
    def _fori__16(k, _acc__17):
        observe(f'_r__{k}__18', bernoulli_logit(beta__ * theta - alpha[
                                                k - 1] * ones__), r[k - 1])
        return None
    _ = fori_loop(1, T + 1, _fori__16, None)


def generated_quantities(*, N, R, culm, T, response, r, ones__, alpha, theta,
                            beta__):
    ...
    def _fori__19(t, _acc__20):
        a = _acc__20
        a = ops_index_update(a, ops_index[t - 1], alpha[t - 1] - mean_alpha)
        return a
...
def map_generated_quantities(_samples, *, N, R, culm, T, response, r, ones__):
    def _generated_quantities(alpha, theta, beta__):
        return generated_quantities(N=N, R=R, culm=culm, T=T,
                                    response=response, r=r, ones__=ones__,
                                    alpha=alpha, theta=theta, beta__=beta__)
...
def parameters_info(*, N, R, culm, T, response, r, ones__):
    return { 'alpha': { 'shape': [T] },'theta': { 'shape': [N] },
             'beta': { 'shape': [] }, }
