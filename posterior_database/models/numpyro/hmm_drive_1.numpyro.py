...
def model(*, N, u, v, K, alpha, tau, rho):
    ...
    lambda__ = sample('lambda', ordered_constrained_improper_uniform(shape=[
    K]))
    ...
    def _fori__8(k, _acc__9):
        gamma__ = _acc__9
        gamma__ = ops_index_update(gamma__, ops_index[1 - 1, k - 1], normal_lpdf(
        u[1 - 1], phi[k - 1], tau) + normal_lpdf(v[1 - 1], lambda__[k - 1],
                                                 rho))
        return gamma__
    ...
    def _fori__10(t, _acc__11):
        ...
        def _fori__12(k, _acc__13):
            ...
            def _fori__14(j, _acc__15):
                ...
                t - 1 - 1, j - 1] + log_real(theta[j - 1, k - 1]) + normal_lpdf(
                u[t - 1], phi[k - 1], tau) + normal_lpdf(v[t - 1],
                                                         lambda__[k - 1], rho))
                return acc
...
def generated_quantities(*, N, u, v, K, alpha, tau, rho, theta1, theta2, phi,
                            lambda__):
    # Transformed parameters
    theta = empty([K, K], dtype=dtype_float)
    theta = ops_index_update(theta, ops_index[1 - 1], theta1)
    ...
    def _fori__17(k, _acc__18):
        best_logp = _acc__18
        best_logp = ops_index_update(best_logp, ops_index[1 - 1, K - 1], normal_lpdf(
        u[1 - 1], phi[k - 1], tau) + normal_lpdf(v[1 - 1], lambda__[k - 1],
                                                 rho))
        return best_logp
...
def map_generated_quantities(_samples, *, N, u, v, K, alpha, tau, rho):
    def _generated_quantities(theta1, theta2, phi, lambda__):
        return generated_quantities(N=N, u=u, v=v, K=K, alpha=alpha, tau=tau,
                                    rho=rho, theta1=theta1, theta2=theta2,
                                    phi=phi, lambda__=lambda__)
