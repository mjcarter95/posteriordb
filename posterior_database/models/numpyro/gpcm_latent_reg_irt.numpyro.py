...
def pcm(y, theta, beta__):
    unsummed = append_row_vector_vector(rep_vector_real_int(array(0.0, dtype=dtype_float),
                                                            1),
                                        theta - beta__)
...
def transformed_data(*, I, N, ii, J, jj, y, K, W):
    ...
    def _fori__12(n, _acc__13):
        ...
        def _then__14(_acc__15):
            m = _acc__15
            m = ops_index_update(m, ops_index[ii[n - 1] - 1], y[n - 1])
            return m
        @jit
        def _else__16(acc):
            return acc
    ...
    pos = ops_index_update(pos, ops_index[1 - 1], 1)
    @jit
    def _fori__17(i, _acc__18):
        pos = _acc__18
        pos = ops_index_update(pos, ops_index[i - 1], m[i - 1 - 1] + pos[
        i - 1 - 1])
        return pos
...
def model(*, I, N, ii, J, jj, y, K, W, m, pos, adj, W_adj):
    ...
    beta__ = ops_index_update(beta__, ops_index[1 - 1:sum_array(m) - 1], beta_free)
    beta__ = ops_index_update(beta__, ops_index[sum_array(m) - 1], - 1 * sum_vector(
    beta_free))
    # Model
    ...
    def _fori__27(n, _acc__28):
        factor(f'_expr__{n}__29', pcm(y[n - 1],
                                      theta[jj[n - 1] - 1] * alpha[ii[
                                                                   n - 1] - 1],
                                      segment_vector_int_int(beta__,
                                                             pos[ii[n - 1] - 1],
                                                             m[ii[n - 1] - 1])))
        return None
...
def map_generated_quantities(_samples, *, I, N, ii, J, jj, y, K, W, m, pos,
    ...
    def _generated_quantities(alpha, beta_free, theta, lambda_adj):
        return generated_quantities(I=I, N=N, ii=ii, J=J, jj=jj, y=y, K=K,
                                    W=W, m=m, pos=pos, adj=adj, W_adj=W_adj,
                                    alpha=alpha, beta_free=beta_free,
                                    theta=theta, lambda_adj=lambda_adj)
...
def parameters_info(*, I, N, ii, J, jj, y, K, W, m, pos, adj, W_adj):
    return { 'alpha': { 'shape': [I] },
             'beta_free': { 'shape': [sum_array(m) - 1] },
