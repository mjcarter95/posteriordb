...
def convert_inputs(inputs):
    ...
    return { 'nyear': nyear, 'nsite': nsite, 'nobs': nobs, 'obs': obs,
             'obsyear': obsyear, 'obssite': obssite, 'nmis': nmis,
             'misyear': misyear, 'missite': missite }

def model(*, nyear, nsite, nobs, obs, obsyear, obssite, nmis, misyear,
             missite):
    # Parameters
...
def generated_quantities(*, nyear, nsite, nobs, obs, obsyear, obssite, nmis,
    ...
    def _fori__6(i, _acc__7):
        mis = _acc__7
        mis = ops_index_update(mis, ops_index[i - 1], poisson_log_rng(
        log_lambda[misyear[i - 1] - 1, missite[i - 1] - 1]))
        return mis
...
def map_generated_quantities(_samples, *, nyear, nsite, nobs, obs, obsyear,
    ...
    def _generated_quantities(alpha, mu_alpha, sd_alpha):
        return generated_quantities(nyear=nyear, nsite=nsite, nobs=nobs,
                                    obs=obs, obsyear=obsyear,
                                    obssite=obssite, nmis=nmis,
                                    misyear=misyear, missite=missite,
                                    alpha=alpha, mu_alpha=mu_alpha,
                                    sd_alpha=sd_alpha)
