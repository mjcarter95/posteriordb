...
def model(*, n_groups, N, group_id, n_scenarios, scenario_id, y):
    ...
    def _fori__1(i, _acc__2):
        y_hat = _acc__2
        y_hat = ops_index_update(y_hat, ops_index[i - 1], a[group_id[
                                                            i - 1] - 1] + b[
        scenario_id[i - 1] - 1])
        return y_hat
...
def map_generated_quantities(_samples, *, n_groups, N, group_id, n_scenarios,
                                          scenario_id, y):
    def _generated_quantities(a, b, mu_a, mu_b, sigma_a, sigma_b, sigma_y):
        return generated_quantities(n_groups=n_groups, N=N,
                                    group_id=group_id,
                                    n_scenarios=n_scenarios,
                                    scenario_id=scenario_id, y=y, a=a, b=b,
                                    mu_a=mu_a, mu_b=mu_b, sigma_a=sigma_a,
                                    sigma_b=sigma_b, sigma_y=sigma_y)
    _f = jit(vmap(_generated_quantities))
    return _f(_samples['a'], _samples['b'], _samples['mu_a'],
              _samples['mu_b'], _samples['sigma_a'], _samples['sigma_b'],
              _samples['sigma_y'])

def parameters_info(*, n_groups, N, group_id, n_scenarios, scenario_id, y):
    return { 'a': { 'shape': [n_groups] },'b': { 'shape': [n_scenarios] },
             'mu_a': { 'shape': [] },'mu_b': { 'shape': [] },
