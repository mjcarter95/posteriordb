...
def model(*, K, N, y):
    ...
    K]))
    mu = sample('mu', positive_ordered_constrained_improper_uniform(shape=[
    K]))
    ...
    def _fori__14(t, _acc__15):
        ...
        def _fori__16(k, _acc__17):
            ...
            (back_ptr, best_logp) = lax_fori_loop(1, K + 1, _fori__18,
                                                  (back_ptr, best_logp))
            return (back_ptr, best_logp)
        (back_ptr, best_logp) = lax_fori_loop(1, K + 1, _fori__16,
                                              (back_ptr, best_logp))
        return (back_ptr, best_logp)
    (back_ptr, best_logp) = lax_fori_loop(2, N + 1, _fori__14,
                                          (back_ptr, best_logp))
...
def map_generated_quantities(_samples, *, K, N, y):
    def _generated_quantities(theta1, theta2, mu):
        return generated_quantities(K=K, N=N, y=y, theta1=theta1,
                                    theta2=theta2, mu=mu)
