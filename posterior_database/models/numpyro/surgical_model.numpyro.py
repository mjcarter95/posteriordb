...
def model(*, N, r, n):
    ...
    def _fori__1(i, _acc__2):
        p = _acc__2
        p = ops_index_update(p, ops_index[i - 1], inv_logit_real(b[i - 1]))
        return p
    p = lax_fori_loop(1, N + 1, _fori__1, p)
    # Model
    observe('_mu__3', normal(array(0.0, dtype=dtype_float),
                             array(1000.0, dtype=dtype_float)), mu)
    observe('_sigmasq__4', inv_gamma(array(0.001, dtype=dtype_float),
                                     array(0.001, dtype=dtype_float)), sigmasq)
