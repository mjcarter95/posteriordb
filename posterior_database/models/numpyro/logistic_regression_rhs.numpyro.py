...
def convert_inputs(inputs):
    ...
    return { 'n': n, 'y': y, 'd': d, 'x': x, 'scale_icept': scale_icept,
             'scale_global': scale_global, 'nu_global': nu_global,
             'nu_local': nu_local, 'slab_scale': slab_scale,
             'slab_df': slab_df }
...
def model(*, n, y, d, x, scale_icept, scale_global, nu_global, nu_local,
    ...
    lambda_tilde = sqrt_vector(true_divide(c ** 2 * square_vector(lambda__), (c ** 2 + tau ** 2 * square_vector(
                               lambda__))))
...
def generated_quantities(*, n, y, d, x, scale_icept, scale_global, nu_global,
                            nu_local, slab_scale, slab_df, beta0, z, tau,
                            lambda__, caux):
    # Transformed parameters
    c = slab_scale * sqrt_real(caux)
    lambda_tilde = sqrt_vector(true_divide(c ** 2 * square_vector(lambda__), (c ** 2 + tau ** 2 * square_vector(
    ...
    def _fori__7(i, _acc__8):
        log_lik = _acc__8
        log_lik = ops_index_update(log_lik, ops_index[i - 1], bernoulli_logit_glm_lpmf(
        array([y[i - 1]], dtype=dtype_long),
        array([x[i - 1]], dtype=dtype_float), beta0, beta__))
        return log_lik
