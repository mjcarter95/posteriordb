...
def convert_inputs(inputs):
    ...
    assoc = array(inputs['assoc'], dtype=dtype_float)
    educ = array(inputs['educ'], dtype=dtype_float)
    return { 'N': N, 'switched': switched, 'dist': dist, 'arsenic': arsenic,
             'assoc': assoc, 'educ': educ }

def transformed_data(*, N, switched, dist, arsenic, assoc, educ):
    # Transformed data
    c_dist100 = true_divide((dist - mean_vector(dist)), array(100.0, dtype=dtype_float))
    c_arsenic = arsenic - mean_vector(arsenic)
    da_inter = c_dist100 * c_arsenic
    educ4 = true_divide(educ, array(4.0, dtype=dtype_float))
    x = transpose(array([c_dist100, c_arsenic, da_inter, assoc, educ4], dtype=dtype_float), 0, 1)
    return { 'c_dist100': c_dist100, 'c_arsenic': c_arsenic,
             'da_inter': da_inter, 'educ4': educ4, 'x': x }

def model(*, N, switched, dist, arsenic, assoc, educ, c_dist100, c_arsenic,
             da_inter, educ4, x):
    # Parameters
...
def parameters_info(*, N, switched, dist, arsenic, assoc, educ, c_dist100,
                       c_arsenic, da_inter, educ4, x):
    return { 'alpha': { 'shape': [] },'beta': { 'shape': [5] }, }
