...
def convert_inputs(inputs):
    ...
    return { 'N': N, 'weight': weight, 'diam1': diam1, 'diam2': diam2,
             'canopy_height': canopy_height, 'group': group }

def transformed_data(*, N, weight, diam1, diam2, canopy_height, group):
    # Transformed data
    log_weight = log_vector(weight)
    log_canopy_volume = log_vector(diam1 * diam2 * canopy_height)
    log_canopy_area = log_vector(diam1 * diam2)
    return { 'log_weight': log_weight,
             'log_canopy_volume': log_canopy_volume,
             'log_canopy_area': log_canopy_area }

def model(*, N, weight, diam1, diam2, canopy_height, group, log_weight,
             log_canopy_volume, log_canopy_area):
    # Parameters
    beta__ = sample('beta', improper_uniform(shape=[4]))
    sigma = sample('sigma', lower_constrained_improper_uniform(0, shape=[]))
    # Model
    observe('_log_weight__1', normal(beta__[1 - 1] + beta__[2 - 1] * log_canopy_volume + beta__[
                                     3 - 1] * log_canopy_area + beta__[
                                     4 - 1] * group, sigma), log_weight)

def parameters_info(*, N, weight, diam1, diam2, canopy_height, group,
                       log_weight, log_canopy_volume, log_canopy_area):
