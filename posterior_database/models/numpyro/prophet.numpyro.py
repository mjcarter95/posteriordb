...
def get_changepoint_matrix(t, t_change, T, S):
    ...
    def _fori__1(i, _acc__2):
        ...
        def _while__3(_acc__4):
            ...
            a_row = ops_index_update(a_row, ops_index[cp_idx - 1], 1)
            cp_idx = cp_idx + 1
            return (a_row, cp_idx)
        (a_row, cp_idx) = lax_while_loop(_cond__5, _while__3, (a_row, cp_idx))
        A = ops_index_update(A, ops_index[i - 1], a_row)
...
def logistic_gamma(k, m, delta, t_change, S):
    ...
    def _fori__7(i, _acc__8):
        ...
        gamma__ = ops_index_update(gamma__, ops_index[i - 1], (t_change[
        i - 1] - m_pr) * (1 - true_divide(k_s[i - 1], k_s[i + 1 - 1])))
...
def logistic_trend(k, m, delta, t, cap, A, t_change, S):
    ...
    return cap * inv_logit_vector((k + matmul(A, delta)) * (t - (m + matmul(A, gamma__))))
...
def convert_inputs(inputs):
    ...
    return { 'T': T, 't': t, 'cap': cap, 'y': y, 'S': S,
             't_change': t_change, 'K': K, 'X': X, 'sigmas': sigmas,
             'tau': tau, 'trend_indicator': trend_indicator, 's_a': s_a,
             's_m': s_m }
...
def model(*, T, t, cap, y, S, t_change, K, X, sigmas, tau, trend_indicator,
    ...
    sigma_obs = sample('sigma_obs', lower_constrained_improper_uniform(0, shape=[]))
    ...
    observe('_sigma_obs__12', normal(0, array(0.5, dtype=dtype_float)), sigma_obs)
    observe('_beta__13', normal(0, sigmas), beta__)
    def _then__14(_acc__15):
        y = _acc__15
        observe('_y__18', normal(linear_trend(k, m, delta, t, A, t_change) * (1 + matmul(X, (beta__ * s_m))) + matmul(X, (beta__ * s_a)),
                                 sigma_obs), y)
        return y
    def _else__16(_acc__17):
        ...
        def _then__19(_acc__20):
            ...
            observe('_y__22', normal(logistic_trend(k, m, delta, t, cap, A,
                                                    t_change, S) * (1 + matmul(X, (beta__ * s_m))) + matmul(X, (beta__ * s_a)),
                                     sigma_obs), y)
            return y
        def _else__21(acc):
            return acc
...
def parameters_info(*, T, t, cap, y, S, t_change, K, X, sigmas, tau,
    ...
    return { 'k': { 'shape': [] },'m': { 'shape': [] },
             'delta': { 'shape': [S] },'sigma_obs': { 'shape': [] },
             'beta': { 'shape': [K] }, }
