...
def transformed_data(*, M, T, y):
    ...
    def _fori__1(i, _acc__2):
        (C, s) = _acc__2
        s = ops_index_update(s, ops_index[i - 1], sum_array(y[i - 1]))
        @jit
...
def model(*, M, T, y, C, s):
    ...
    logit_p = ops_index_update(logit_p, ops_index[:, 1 - 1], alpha[1 - 1] + eps)
    @jit
    def _fori__6(i, _acc__7):
        ...
        def _fori__8(j, _acc__9):
            logit_p = _acc__9
            logit_p = ops_index_update(logit_p, ops_index[i - 1, j - 1], alpha[
            j - 1] + eps[i - 1] + gamma__ * y[i - 1, j - 1 - 1])
            return logit_p
    ...
    def _fori__12(i, _acc__13):
        def _then__14(_acc__15):
            factor(f'_expr__{i}__18', bernoulli_lpmf(1, omega) + bernoulli_logit_lpmf(
            y[i - 1], logit_p[i - 1]))
            return None
        def _else__16(_acc__17):
            factor(f'_expr__{i}__19', log_sum_exp_real_real(bernoulli_lpmf(
                                                            1, omega) + bernoulli_logit_lpmf(
                                                            y[i - 1],
                                                            logit_p[i - 1]),
                                                            bernoulli_lpmf(
                                                            0, omega)))
...
def generated_quantities(*, M, T, y, C, s, omega, mean_p, gamma__, sigma,
    ...
    def _fori__24(i, _acc__25):
        ...
        def _else__28(_acc__29):
            ...
            z = ops_index_update(z, ops_index[i - 1], bernoulli_rng(true_divide(omega * pr, (omega * pr + (1 - omega)))))
            return z
...
def map_generated_quantities(_samples, *, M, T, y, C, s):
    def _generated_quantities(omega, mean_p, gamma__, sigma, eps_raw):
        return generated_quantities(M=M, T=T, y=y, C=C, s=s, omega=omega,
                                    mean_p=mean_p, gamma__=gamma__,
                                    sigma=sigma, eps_raw=eps_raw)
...
def parameters_info(*, M, T, y, C, s):
    return { 'omega': { 'shape': [] },'mean_p': { 'shape': [T] },
