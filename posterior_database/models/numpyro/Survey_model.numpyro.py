...
def model(*, nmax, m, k, nmin):
    ...
    def _fori__1(n, _acc__2):
        ...
        def _then__3(_acc__4):
            lp_parts = _acc__4
            lp_parts = ops_index_update(lp_parts, ops_index[n - 1], log_real(
            true_divide(array(1.0, dtype=dtype_float), nmax)) + negative_infinity(
            ))
            return lp_parts
        @jit
        def _else__5(_acc__6):
            lp_parts = _acc__6
            lp_parts = ops_index_update(lp_parts, ops_index[n - 1], log_real(
            true_divide(array(1.0, dtype=dtype_float), nmax)) + binomial_lpmf(
            k, n, theta))
            return lp_parts
...
def map_generated_quantities(_samples, *, nmax, m, k, nmin):
    def _generated_quantities(theta):
        return generated_quantities(nmax=nmax, m=m, k=k, nmin=nmin,
                                    theta=theta)
