...
def model(*, n_trials, n_dogs, y):
    ...
    def _fori__1(j, _acc__2):
        ...
        def _fori__5(t, _acc__6):
            p = _acc__6
            p = ops_index_update(p, ops_index[j - 1, t - 1], inv_logit_real(
            beta__[1 - 1] * n_avoid[j - 1, t - 1] + beta__[2 - 1] * n_shock[
            j - 1, t - 1]))
            return p
    ...
    def _fori__9(i, _acc__10):
        def _fori__11(j, _acc__12):
            observe(f'_y__{j}__{i}__13', bernoulli(p[i - 1, j - 1]), y[
            i - 1, j - 1])
            return None
...
def generated_quantities(*, n_trials, n_dogs, y, beta__):
    ...
    def _fori__14(j, _acc__15):
        ...
        return (n_avoid, n_shock, p)
    (n_avoid, n_shock, p) = lax_fori_loop(1, n_dogs + 1, _fori__14,
                                          (n_avoid, n_shock, p))
