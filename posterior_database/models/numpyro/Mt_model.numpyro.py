...
from stannumpyro.stanlib import log_sum_exp_real_real, prod_vector, sum_array
...
def model(*, M, T, y, C, s):
    ...
    p = sample('p', uniform(0 * ones([T]), 1))
    # Model
    def _fori__6(i, _acc__7):
        def _then__8(_acc__9):
            factor(f'_expr__{i}__12', bernoulli_lpmf(1, omega) + bernoulli_lpmf(
            y[i - 1], p))
            return None
...
def generated_quantities(*, M, T, y, C, s, omega, p):
    ...
    pr = prod_vector(1 - p)
    omega_nd = true_divide((omega * pr), (omega * pr + (1 - omega)))
    N = C + binomial_rng(M - C, omega_nd)
    return { 'pr': pr, 'omega_nd': omega_nd, 'N': N }
