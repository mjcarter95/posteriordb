...
def model(*, M, T, y, C, s):
    ...
    def _fori__6(j, _acc__7):
        logit_p = _acc__7
        logit_p = ops_index_update(logit_p, ops_index[:, j - 1], mean_lp[
        j - 1] + eps)
        return logit_p
    ...
    def _fori__9(i, _acc__10):
        def _then__11(_acc__12):
            factor(f'_expr__{i}__15', bernoulli_lpmf(1, omega) + bernoulli_logit_lpmf(
            y[i - 1], logit_p[i - 1]))
            return None
...
def generated_quantities(*, M, T, y, C, s, omega, mean_p, sigma, eps_raw):
    ...
    N = sum_array(z)
    return { 'eps': eps, 'mean_lp': mean_lp, 'logit_p': logit_p, 'p': p,
             'z': z, 'N': N }
