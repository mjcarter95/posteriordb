...
def two_pool_feedback(t, C, theta, x_r, x_i):
    ...
    dC_dt = ops_index_update(dC_dt, ops_index[1 - 1], - k1 * C[1 - 1] + alpha12 * k2 * C[
    2 - 1])
    dC_dt = ops_index_update(dC_dt, ops_index[2 - 1], - k2 * C[2 - 1] + alpha21 * k1 * C[
    1 - 1])
    return dC_dt
...
def evolved_CO2(N_t, t0, ts, gamma__, totalC_t0, k1, k2, alpha21, alpha12,
    ...
    C_t0 = ops_index_update(C_t0, ops_index[1 - 1], gamma__ * totalC_t0)
    C_t0 = ops_index_update(C_t0, ops_index[2 - 1], (1 - gamma__) * totalC_t0)
    theta = empty([4], dtype=dtype_float)
    theta = ops_index_update(theta, ops_index[1 - 1], k1)
    theta = ops_index_update(theta, ops_index[2 - 1], k2)
    theta = ops_index_update(theta, ops_index[3 - 1], alpha21)
    theta = ops_index_update(theta, ops_index[4 - 1], alpha12)
    ...
    def _fori__1(t, _acc__2):
        eCO2_hat = _acc__2
        eCO2_hat = ops_index_update(eCO2_hat, ops_index[t - 1], totalC_t0 - sum_array(
        C_hat[t - 1]))
        return eCO2_hat
...
def model(*, totalC_t0, t0, N_t, ts, eCO2mean, x_r, x_i):
    ...
    def _fori__9(t, _acc__10):
        observe(f'_eCO2mean__{t}__11', normal(eCO2_hat[t - 1], sigma), eCO2mean[
        t - 1])
        return None
...
def map_generated_quantities(_samples, *, totalC_t0, t0, N_t, ts, eCO2mean,
    ...
    def _generated_quantities(k1, k2, alpha21, alpha12, gamma__, sigma):
        return generated_quantities(totalC_t0=totalC_t0, t0=t0, N_t=N_t,
                                    ts=ts, eCO2mean=eCO2mean, x_r=x_r,
                                    x_i=x_i, k1=k1, k2=k2, alpha21=alpha21,
                                    alpha12=alpha12, gamma__=gamma__,
                                    sigma=sigma)
    _f = jit(vmap(_generated_quantities))
    return _f(_samples['k1'], _samples['k2'], _samples['alpha21'],
              _samples['alpha12'], _samples['gamma'], _samples['sigma'])

def parameters_info(*, totalC_t0, t0, N_t, ts, eCO2mean, x_r, x_i):
    return { 'k1': { 'shape': [] },'k2': { 'shape': [] },
             'alpha21': { 'shape': [] },'alpha12': { 'shape': [] },
             'gamma': { 'shape': [] },'sigma': { 'shape': [] }, }
