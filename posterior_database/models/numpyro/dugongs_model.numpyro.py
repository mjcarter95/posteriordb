...
def model(*, N, x, Y):
    ...
    lambda__ = sample('lambda', uniform(array(.5, dtype=dtype_float), 1))
    ...
    def _fori__1(i, _acc__2):
        m = _acc__2
        m = ops_index_update(m, ops_index[i - 1], alpha - beta__ * pow_real_real(
        lambda__, x[i - 1]))
        return m
    ...
    observe('_lambda__6', uniform(array(.5, dtype=dtype_float), 1), lambda__)
    observe('_tau__7', gamma(array(.0001, dtype=dtype_float),
                             array(.0001, dtype=dtype_float)), tau)
...
def map_generated_quantities(_samples, *, N, x, Y):
    def _generated_quantities(alpha, beta__, lambda__, tau):
        return generated_quantities(N=N, x=x, Y=Y, alpha=alpha,
                                    beta__=beta__, lambda__=lambda__, tau=tau)
