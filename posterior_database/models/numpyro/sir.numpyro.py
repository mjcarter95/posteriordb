...
def simple_SIR(t, y, theta, x_r, x_i):
    ...
    dydt = ops_index_update(dydt, ops_index[1 - 1], true_divide(- theta[
    1 - 1] * y[4 - 1], (y[4 - 1] + theta[2 - 1])) * y[1 - 1])
    dydt = ops_index_update(dydt, ops_index[2 - 1], true_divide(theta[
    1 - 1] * y[4 - 1], (y[4 - 1] + theta[2 - 1])) * y[1 - 1] - theta[
    3 - 1] * y[2 - 1])
    dydt = ops_index_update(dydt, ops_index[3 - 1], theta[3 - 1] * y[
    2 - 1])
    dydt = ops_index_update(dydt, ops_index[4 - 1], theta[4 - 1] * y[
    2 - 1] - theta[5 - 1] * y[4 - 1])
    return dydt
...
def convert_inputs(inputs):
    ...
    return { 'N_t': N_t, 't': t, 'y0': y0, 'stoi_hat': stoi_hat,
             'B_hat': B_hat }
...
def model(*, N_t, t, y0, stoi_hat, B_hat, t0, kappa, x_r, x_i):
    ...
    def _fori__6(n, _acc__7):
        observe(f'_stoi_hat__{n}__8', poisson(y[n - 1 - 1, 1 - 1] - y[
                                              n - 1, 1 - 1]), stoi_hat[
        n - 1])
        return None
    _ = fori_loop(2, N_t + 1, _fori__6, None)
    observe('_B_hat__9', lognormal(log_vector(col_matrix_int(to_matrix_array(
                                                             y), 4)),
                                   array(0.15, dtype=dtype_float)), B_hat)


def generated_quantities(*, N_t, t, y0, stoi_hat, B_hat, t0, kappa, x_r, x_i,
                            beta__, gamma__, xi, delta):
    # Transformed parameters
    theta = array([beta__, kappa, gamma__, xi, delta], dtype=dtype_float)
    y = integrate_ode_rk45_function_array_real_array_array_array_array(
    simple_SIR, y0, t0, t, theta, x_r, x_i)
...
def map_generated_quantities(_samples, *, N_t, t, y0, stoi_hat, B_hat, t0,
    ...
    def _generated_quantities(beta__, gamma__, xi, delta):
        return generated_quantities(N_t=N_t, t=t, y0=y0, stoi_hat=stoi_hat,
                                    B_hat=B_hat, t0=t0, kappa=kappa, x_r=x_r,
                                    x_i=x_i, beta__=beta__, gamma__=gamma__,
                                    xi=xi, delta=delta)
    _f = jit(vmap(_generated_quantities))
    return _f(_samples['beta'], _samples['gamma'], _samples['xi'],
              _samples['delta'])

def parameters_info(*, N_t, t, y0, stoi_hat, B_hat, t0, kappa, x_r, x_i):
    return { 'beta': { 'shape': [] },'gamma': { 'shape': [] },
             'xi': { 'shape': [] },'delta': { 'shape': [] }, }
