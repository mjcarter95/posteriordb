...
def transformed_data(*, n_dogs, n_trials, y):
    ...
    def _fori__1(j, _acc__2):
        ...
        prev_shock = ops_index_update(prev_shock, ops_index[j - 1, 1 - 1], 0)
        prev_avoid = ops_index_update(prev_avoid, ops_index[j - 1, 1 - 1], 0)
        @jit
        def _fori__3(t, _acc__4):
            (prev_avoid, prev_shock) = _acc__4
            prev_shock = ops_index_update(prev_shock, ops_index[j - 1, t - 1], prev_shock[
            j - 1, t - 1 - 1] + y[j - 1, t - 1 - 1])
            prev_avoid = ops_index_update(prev_avoid, ops_index[j - 1, t - 1], prev_avoid[
        ...
        return (prev_avoid, prev_shock)
    (prev_avoid, prev_shock) = lax_fori_loop(1, J + 1, _fori__1,
                                             (prev_avoid, prev_shock))
    return { 'J': J, 'T': T, 'prev_shock': prev_shock,
             'prev_avoid': prev_avoid }

def model(*, n_dogs, n_trials, y, J, T, prev_shock, prev_avoid):
    # Parameters
    mu_logit_ab = sample('mu_logit_ab', improper_uniform(shape=[2]))
    sigma_logit_ab = sample('sigma_logit_ab', lower_constrained_improper_uniform(0, shape=[
    2]))
    L_logit_ab = sample('L_logit_ab', cholesky_factor_corr_constrained_improper_uniform(shape=[
    2, 2]))
    z = sample('z', improper_uniform(shape=[J, 2]))
    # Transformed parameters
    logit_ab = matmul(rep_vector_int_int(1, J), mu_logit_ab) + matmul(z, diag_pre_multiply_vector_matrix(
    sigma_logit_ab, L_logit_ab))
...
def generated_quantities(*, n_dogs, n_trials, y, J, T, prev_shock,
                            prev_avoid, mu_logit_ab, sigma_logit_ab,
                            L_logit_ab, z):
    # Transformed parameters
    logit_ab = matmul(rep_vector_int_int(1, J), mu_logit_ab) + matmul(z, diag_pre_multiply_vector_matrix(
    sigma_logit_ab, L_logit_ab))
    ...
    def _fori__14(j, _acc__15):
        ...
        prev_shock_rep = 0
        prev_avoid_rep = 0
        y_rep = ops_index_update(y_rep, ops_index[j - 1, 1 - 1], 1)
        @jit
        def _fori__16(t, _acc__17):
            ...
            y_rep = ops_index_update(y_rep, ops_index[j - 1, t - 1], bernoulli_rng(
            p_rep))
            return (p_rep, prev_avoid_rep, prev_shock_rep, y_rep)
        (p_rep, prev_avoid_rep, prev_shock_rep, y_rep) = lax_fori_loop(
        2, T + 1, _fori__16, (p_rep, prev_avoid_rep, prev_shock_rep, y_rep))
        return (p_rep, prev_avoid_rep, prev_shock_rep, y_rep)
    ...
    return { 'logit_ab': logit_ab, 'Omega_logit_ab': Omega_logit_ab,
             'Sigma_logit_ab': Sigma_logit_ab, 'a': a, 'b': b,
             'y_rep': y_rep, 'prev_shock_rep': prev_shock_rep,
...
def map_generated_quantities(_samples, *, n_dogs, n_trials, y, J, T,
    ...
    def _generated_quantities(mu_logit_ab, sigma_logit_ab, L_logit_ab, z):
        return generated_quantities(n_dogs=n_dogs, n_trials=n_trials, y=y,
                                    J=J, T=T, prev_shock=prev_shock,
                                    prev_avoid=prev_avoid,
                                    mu_logit_ab=mu_logit_ab,
                                    sigma_logit_ab=sigma_logit_ab,
                                    L_logit_ab=L_logit_ab, z=z)
    _f = jit(vmap(_generated_quantities))
    return _f(_samples['mu_logit_ab'], _samples['sigma_logit_ab'],
              _samples['L_logit_ab'], _samples['z'])

def parameters_info(*, n_dogs, n_trials, y, J, T, prev_shock, prev_avoid):
    return { 'mu_logit_ab': { 'shape': [2] },
             'sigma_logit_ab': { 'shape': [2] },
