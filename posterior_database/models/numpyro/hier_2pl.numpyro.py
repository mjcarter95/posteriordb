...
def model(*, I, N, ii, J, jj, y):
    ...
    2]))
    L_Omega = sample('L_Omega', cholesky_factor_corr_constrained_improper_uniform(shape=[
    2, 2]))
    ...
    def _fori__1(i, _acc__2):
        ...
        xi = ops_index_update(xi, ops_index[i - 1, 1 - 1], xi1[i - 1])
        xi = ops_index_update(xi, ops_index[i - 1, 2 - 1], xi2[i - 1])
        alpha = ops_index_update(alpha, ops_index[i - 1], exp_real(xi[
                                                                   i - 1,
                                                                   1 - 1]))
        beta__ = ops_index_update(beta__, ops_index[i - 1], xi[i - 1, 2 - 1])
        return (alpha, beta__, xi)
    (alpha, beta__, xi) = lax_fori_loop(1, I + 1, _fori__1,
                                        (alpha, beta__, xi))
    # Model
    ...
    observe('_tau__9', exponential(array(.1, dtype=dtype_float)), tau[
    1 - 1])
    observe('_mu__10', normal(0, 5), mu[2 - 1])
    observe('_tau__11', exponential(array(.1, dtype=dtype_float)), tau[
    2 - 1])
    observe('_y__12', bernoulli_logit(alpha[ii - 1] * (theta[jj - 1] - beta__[
                                      ii - 1])), y)
...
def generated_quantities(*, I, N, ii, J, jj, y, theta, xi1, xi2, mu, tau,
    ...
    def _fori__13(i, _acc__14):
        ...
        beta__ = ops_index_update(beta__, ops_index[i - 1], xi[i - 1, 2 - 1])
        return (alpha, beta__, xi)
    (alpha, beta__, xi) = lax_fori_loop(1, I + 1, _fori__13,
                                        (alpha, beta__, xi))
...
def map_generated_quantities(_samples, *, I, N, ii, J, jj, y):
    def _generated_quantities(theta, xi1, xi2, mu, tau, L_Omega):
        return generated_quantities(I=I, N=N, ii=ii, J=J, jj=jj, y=y,
                                    theta=theta, xi1=xi1, xi2=xi2, mu=mu,
                                    tau=tau, L_Omega=L_Omega)
    _f = jit(vmap(_generated_quantities))
    return _f(_samples['theta'], _samples['xi1'], _samples['xi2'],
              _samples['mu'], _samples['tau'], _samples['L_Omega'])

def parameters_info(*, I, N, ii, J, jj, y):
    return { 'theta': { 'shape': [J] },'xi1': { 'shape': [I] },
             'xi2': { 'shape': [I] },'mu': { 'shape': [2] },
             'tau': { 'shape': [2] },'L_Omega': { 'shape': [2, 2] }, }
