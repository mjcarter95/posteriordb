...
def model(*, I, J, y):
    # Parameters
    sigma_theta = sample('sigma_theta', lower_constrained_improper_uniform(0, shape=[]))
    ...
    def _fori__8(i, _acc__9):
        observe(f'_y__{i}__10', bernoulli_logit(a[i - 1] * (theta - b[i - 1])), y[
        i - 1])
        return None
...
def parameters_info(*, I, J, y):
    return { 'sigma_theta': { 'shape': [] },'theta': { 'shape': [J] },
             'sigma_a': { 'shape': [] },'a': { 'shape': [I] },
             'mu_b': { 'shape': [] },'sigma_b': { 'shape': [] },
             'b': { 'shape': [I] }, }
