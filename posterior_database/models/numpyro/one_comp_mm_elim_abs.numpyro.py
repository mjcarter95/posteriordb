...
def one_comp_mm_elim_abs(t, y, theta, x_r, x_i):
    ...
    dydt = ops_index_update(dydt, ops_index[1 - 1], dose - elim)
    return dydt
...
def convert_inputs(inputs):
    ...
    return { 't0': t0, 'D': D, 'V': V, 'N_t': N_t, 'times': times,
             'C_hat': C_hat }
...
def generated_quantities(*, t0, D, V, N_t, times, C_hat, C0, x_r, x_i, k_a,
    ...
    def _fori__11(n, _acc__12):
        C_ppc = _acc__12
        C_ppc = ops_index_update(C_ppc, ops_index[n - 1], lognormal_rng(
        log_real(C[n - 1, 1 - 1]), sigma))
        return C_ppc
...
def map_generated_quantities(_samples, *, t0, D, V, N_t, times, C_hat, C0,
    ...
    def _generated_quantities(k_a, K_m, V_m, sigma):
        return generated_quantities(t0=t0, D=D, V=V, N_t=N_t, times=times,
                                    C_hat=C_hat, C0=C0, x_r=x_r, x_i=x_i,
                                    k_a=k_a, K_m=K_m, V_m=V_m, sigma=sigma)
    _f = jit(vmap(_generated_quantities))
    return _f(_samples['k_a'], _samples['K_m'], _samples['V_m'],
              _samples['sigma'])

def parameters_info(*, t0, D, V, N_t, times, C_hat, C0, x_r, x_i):
    return { 'k_a': { 'shape': [] },'K_m': { 'shape': [] },
             'V_m': { 'shape': [] },'sigma': { 'shape': [] }, }
