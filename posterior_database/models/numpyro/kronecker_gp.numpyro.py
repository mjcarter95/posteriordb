...
def calculate_eigenvalues(A, B, n1, n2, sigma2):
    ...
    def _fori__1(i, _acc__2):
        ...
        def _fori__3(j, _acc__4):
            e__ = _acc__4
            e__ = ops_index_update(e__, ops_index[i - 1, j - 1], A[i - 1] * B[
            j - 1] + sigma2)
            return e__
...
def transformed_data(*, n2, x1, n1, y):
    ...
    def _fori__5(i, _acc__6):
        xd = _acc__6
        xd = ops_index_update(xd, ops_index[i - 1, i - 1], 0)
        @jit
        def _fori__7(j, _acc__8):
            xd = _acc__8
            xd = ops_index_update(xd, ops_index[i - 1, j - 1], - ((x1[
            i - 1] - x1[j - 1]) ** 2))
            xd = ops_index_update(xd, ops_index[j - 1, i - 1], xd[i - 1,
                                                                  j - 1])
            return xd
...
def model(*, n2, x1, n1, y, xd):
    ...
    n2, n2]))
    sigma1 = sample('sigma1', lower_constrained_improper_uniform(array(0.00001, dtype=dtype_float), shape=[]))
    ...
    def _fori__9(i, _acc__10):
        Sigma1 = _acc__10
        Sigma1 = ops_index_update(Sigma1, ops_index[i - 1, i - 1], Sigma1[
        i - 1, i - 1] + array(.00001, dtype=dtype_float))
        return Sigma1
    ...
    factor('_expr__15', - array(0.5, dtype=dtype_float) * sum_matrix(
    y * kron_mvprod(Q1, Q2,
                    true_divide(kron_mvprod(transpose_matrix(Q1),
                                            transpose_matrix(Q2), y), eigenvalues))) - array(0.5, dtype=dtype_float) * sum_matrix(
    log_matrix(eigenvalues)))
...
def generated_quantities(*, n2, x1, n1, y, xd, var1, bw1, L, sigma1):
    ...
    return { 'Lambda': Lambda, 'Sigma1': Sigma1, 'Q1': Q1, 'R1': R1,
             'Q2': Q2, 'R2': R2, 'eigenvalues': eigenvalues }

def map_generated_quantities(_samples, *, n2, x1, n1, y, xd):
    def _generated_quantities(var1, bw1, L, sigma1):
        return generated_quantities(n2=n2, x1=x1, n1=n1, y=y, xd=xd,
                                    var1=var1, bw1=bw1, L=L, sigma1=sigma1)
    _f = jit(vmap(_generated_quantities))
    return _f(_samples['var1'], _samples['bw1'], _samples['L'],
              _samples['sigma1'])

def parameters_info(*, n2, x1, n1, y, xd):
    return { 'var1': { 'shape': [] },'bw1': { 'shape': [] },
             'L': { 'shape': [n2, n2] },'sigma1': { 'shape': [] }, }
