    ...
    def _fori__2(i, _acc__3):
        ...
        def _fori__4(j, _acc__5):
            ...
            def _fori__6(k, _acc__7):
                Q = _acc__7
                Q = ops_index_update(Q, ops_index[i - 1, j - 1, k - 1], inv_logit_real(
                delta[j - 1] * (theta[i - 1] - gamma__[j - 1, k - 1])))
                return Q
            Q = lax_fori_loop(1, (ncat[j - 1] - 1) + 1, _fori__6, Q)
            p = ops_index_update(p, ops_index[i - 1, j - 1, 1 - 1], 1 - Q[
            i - 1, j - 1, 1 - 1])
            @jit
            def _fori__8(k, _acc__9):
                p = _acc__9
                p = ops_index_update(p, ops_index[i - 1, j - 1, k - 1], Q[
                i - 1, j - 1, k - 1 - 1] - Q[i - 1, j - 1, k - 1])
                return p
            p = lax_fori_loop(2, (ncat[j - 1] - 1) + 1, _fori__8, p)
            p = ops_index_update(p, ops_index[i - 1, j - 1, ncat[j - 1] - 1], Q[
            i - 1, j - 1, ncat[j - 1] - 1 - 1])
            ...
            def _else__12(acc):
                return acc
            _ = numpyro_cond(grade[i - 1, j - 1] != - 1,
                             _then__10, _else__12, None)
            return (Q, p)
