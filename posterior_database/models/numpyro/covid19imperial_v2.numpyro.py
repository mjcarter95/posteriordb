...
def convert_inputs(inputs):
    ...
    return { 'N0': N0, 'M': M, 'N': N, 'N2': N2, 'cases': cases,
             'deaths': deaths, 'f': f, 'P': P, 'X': X,
             'EpidemicStart': EpidemicStart, 'pop': pop, 'SI': SI }
...
def transformed_data(*, N0, M, N, N2, cases, deaths, f, P, X, EpidemicStart,
    ...
    def _fori__1(i, _acc__2):
        SI_rev = _acc__2
        SI_rev = ops_index_update(SI_rev, ops_index[i - 1], SI[N2 - i + 1 - 1])
        return SI_rev
    ...
    def _fori__3(m, _acc__4):
        ...
        def _fori__5(i, _acc__6):
            f_rev = _acc__6
            f_rev = ops_index_update(f_rev, ops_index[m - 1, i - 1], f[
            N2 - i + 1 - 1, m - 1])
            return f_rev
...
def model(*, N0, M, N, N2, cases, deaths, f, P, X, EpidemicStart, pop, SI,
    ...
    alpha_hier = sample('alpha_hier', lower_constrained_improper_uniform(0, shape=[
    P]))
    ...
    ifr_noise = sample('ifr_noise', lower_constrained_improper_uniform(0, shape=[
    M]))
    ...
    def _fori__7(i, _acc__8):
        alpha = _acc__8
        alpha = ops_index_update(alpha, ops_index[i - 1], alpha_hier[
        i - 1] - (true_divide(log_real(array(1.05, dtype=dtype_float)), array(6.0, dtype=dtype_float))))
        return alpha
    alpha = lax_fori_loop(1, P + 1, _fori__7, alpha)
    @jit
    def _fori__9(m, _acc__10):
        (E_deaths, Rt, Rt_adj, cumm_sum, prediction) = _acc__10
        prediction = ops_index_update(prediction, ops_index[1 - 1:N0, m - 1], rep_vector_real_int(
        y[m - 1], N0))
        cumm_sum = ops_index_update(cumm_sum, ops_index[2 - 1:N0, m - 1], cumulative_sum_vector(
        prediction[2 - 1:N0, m - 1]))
        Rt = ops_index_update(Rt, ops_index[:, m - 1], mu[m - 1] * exp_vector(
        matmul(- X[m - 1], alpha)))
        Rt_adj = ops_index_update(Rt_adj, ops_index[1 - 1:N0, m - 1], Rt[
        1 - 1:N0, m - 1])
        @jit
        def _fori__11(i, _acc__12):
            ...
            convolution = dot_product_vector_vector(sub_col_matrix_int_int_int(
                                                    prediction, 1, m, i - 1),
                                                    tail_vector_int(SI_rev,
                                                                    i - 1))
            cumm_sum = ops_index_update(cumm_sum, ops_index[i - 1, m - 1], cumm_sum[
            i - 1 - 1, m - 1] + prediction[i - 1 - 1, m - 1])
            Rt_adj = ops_index_update(Rt_adj, ops_index[i - 1, m - 1], (true_divide((pop[
            m - 1] - cumm_sum[i - 1, m - 1]), pop[m - 1])) * Rt[i - 1, m - 1])
            prediction = ops_index_update(prediction, ops_index[i - 1, m - 1], Rt_adj[
            i - 1, m - 1] * convolution)
        ...
        (Rt_adj, cumm_sum, prediction) = lax_fori_loop((N0 + 1), N2 + 1,
                                                       ...
                                                       (Rt_adj, cumm_sum,
                                                        prediction))
        E_deaths = ops_index_update(E_deaths, ops_index[1 - 1, m - 1], array(1e-15, dtype=dtype_float) * prediction[
        1 - 1, m - 1])
        @jit
        def _fori__13(i, _acc__14):
            E_deaths = _acc__14
            E_deaths = ops_index_update(E_deaths, ops_index[i - 1, m - 1], ifr_noise[
            m - 1] * dot_product_vector_vector(sub_col_matrix_int_int_int(
                                               prediction, 1, m, i - 1),
                                               tail_vector_int(f_rev[
                                                               m - 1], i - 1)))
            return E_deaths
    ...
    observe('_alpha_hier__22', gamma(array(.1667, dtype=dtype_float), 1), alpha_hier)
    observe('_ifr_noise__23', normal(1, array(0.1, dtype=dtype_float)), ifr_noise)
    def _fori__24(m, _acc__25):
        observe(f'_deaths__{m}__26', neg_binomial_2(E_deaths[EpidemicStart[
                                                             m - 1] - 1:N[
                                                             m - 1], m - 1],
                                                    phi), deaths[EpidemicStart[
                                                                 m - 1] - 1:N[
                                                                 m - 1],
                                                                 m - 1])
        return None
...
def generated_quantities(*, N0, M, N, N2, cases, deaths, f, P, X,
                            EpidemicStart, pop, SI, SI_rev, f_rev, mu,
                            alpha_hier, kappa, y, phi, tau, ifr_noise):
    # Transformed parameters
    prediction = rep_matrix_int_int_int(0, N2, M)
    E_deaths = rep_matrix_int_int_int(0, N2, M)
    Rt = rep_matrix_int_int_int(0, N2, M)
    ...
    def _fori__35(m, _acc__36):
        ...
        def _fori__37(i, _acc__38):
            cumm_sum0 = _acc__38
            cumm_sum0 = ops_index_update(cumm_sum0, ops_index[i - 1, m - 1], cumm_sum0[
            i - 1 - 1, m - 1] + y[m - 1])
            return cumm_sum0
        cumm_sum0 = lax_fori_loop(2, N0 + 1, _fori__37, cumm_sum0)
        prediction0 = ops_index_update(prediction0, ops_index[1 - 1:N0, m - 1], rep_vector_real_int(
        y[m - 1], N0))
        @jit
        def _fori__39(i, _acc__40):
            ...
            def _fori__41(j, _acc__42):
                ...
                i - j - 1]
                return convolution0
            convolution0 = lax_fori_loop(1, (i - 1) + 1, _fori__41,
                                         convolution0)
            cumm_sum0 = ops_index_update(cumm_sum0, ops_index[i - 1, m - 1], cumm_sum0[
            i - 1 - 1, m - 1] + prediction0[i - 1 - 1, m - 1])
            prediction0 = ops_index_update(prediction0, ops_index[i - 1,
                                                                  m - 1], (true_divide((pop[
            m - 1] - cumm_sum0[i - 1, m - 1]), pop[m - 1])) * mu[m - 1] * convolution0)
            return (cumm_sum0, prediction0)
        (cumm_sum0, prediction0) = lax_fori_loop((N0 + 1), N2 + 1, _fori__39,
                                                 (cumm_sum0, prediction0))
        E_deaths0 = ops_index_update(E_deaths0, ops_index[1 - 1, m - 1], uniform_rng(
        array(1e-16, dtype=dtype_float), array(1e-15, dtype=dtype_float)))
        @jit
        def _fori__43(i, _acc__44):
            ...
            def _fori__45(j, _acc__46):
                E_deaths0 = _acc__46
                E_deaths0 = ops_index_update(E_deaths0, ops_index[i - 1,
                                                                  m - 1], E_deaths0[
                i - 1, m - 1] + prediction0[j - 1, m - 1] * f[i - j - 1,
                                                              m - 1] * ifr_noise[
                m - 1])
                return E_deaths0
    ...
    return { 'prediction': prediction, 'E_deaths': E_deaths, 'Rt': Rt,
             ...
             'prediction0': prediction0, 'E_deaths0': E_deaths0,
             'cumm_sum0': cumm_sum0 }

def map_generated_quantities(_samples, *, N0, M, N, N2, cases, deaths, f, P,
                                          X, EpidemicStart, pop, SI, SI_rev,
                                          f_rev):
    def _generated_quantities(mu, alpha_hier, kappa, y, phi, tau, ifr_noise):
        return generated_quantities(N0=N0, M=M, N=N, N2=N2, cases=cases,
                                    deaths=deaths, f=f, P=P, X=X,
                                    EpidemicStart=EpidemicStart, pop=pop,
                                    SI=SI, SI_rev=SI_rev, f_rev=f_rev, mu=mu,
                                    alpha_hier=alpha_hier, kappa=kappa, y=y,
                                    phi=phi, tau=tau, ifr_noise=ifr_noise)
    _f = jit(vmap(_generated_quantities))
    return _f(_samples['mu'], _samples['alpha_hier'], _samples['kappa'],
              _samples['y'], _samples['phi'], _samples['tau'],
              _samples['ifr_noise'])

def parameters_info(*, N0, M, N, N2, cases, deaths, f, P, X, EpidemicStart,
                       pop, SI, SI_rev, f_rev):
    return { 'mu': { 'shape': [M] },'alpha_hier': { 'shape': [P] },
             'kappa': { 'shape': [] },'y': { 'shape': [M] },
             'phi': { 'shape': [] },'tau': { 'shape': [] },
             'ifr_noise': { 'shape': [M] }, }
