...
def model(*, T, y):
    ...
    nu = ops_index_update(nu, ops_index[1 - 1], mu + phi * mu)
    err = empty([T], dtype=dtype_float)
    err = ops_index_update(err, ops_index[1 - 1], y[1 - 1] - nu[1 - 1])
    @jit
    def _fori__5(t, _acc__6):
        (err, nu) = _acc__6
        nu = ops_index_update(nu, ops_index[t - 1], mu + phi * y[t - 1 - 1] + theta * err[
        t - 1 - 1])
        err = ops_index_update(err, ops_index[t - 1], y[t - 1] - nu[t - 1])
        return (err, nu)
    (err, nu) = lax_fori_loop(2, T + 1, _fori__5, (err, nu))
    observe('_err__7', normal(0, sigma), err)

def parameters_info(*, T, y):
    return { 'mu': { 'shape': [] },'phi': { 'shape': [] },
             'theta': { 'shape': [] },'sigma': { 'shape': [] }, }
