...
def model(*, K, T, y):
    ...
    logalpha = logalpha.at[1 - 1].set(log_vector(pi1) + normal_lpdf(y[1 - 1
        ], mu, sigma))
    ...
    def _fori__1(t, _acc__2):
        ...
        def _fori__3(j, _acc__4):
            ...
            def _fori__5(i, _acc__6):
                ...
                accumulator = accumulator.at[i - 1].set(logalpha[t - 1 - 1,
                    i - 1] + log_real(A[i - 1, j - 1]) + normal_lpdf(y[t - 
                    1], mu[j - 1], sigma[j - 1]))
                return accumulator
        ...
        return accumulator, logalpha
    accumulator, logalpha = lax_fori_loop(2, T + 1, _fori__1, (accumulator,
        logalpha))
...
def generated_quantities(*, K, T, y, pi1, A, mu, sigma):
    ...
    def _fori__8(t, _acc__9):
        ...
        accumulator, logalpha = lax_fori_loop(1, K + 1, _fori__10, (
            accumulator, logalpha))
        return accumulator, logalpha
    accumulator, logalpha = lax_fori_loop(2, T + 1, _fori__8, (accumulator,
        logalpha))
    ...
    def _fori__18(tforward, _acc__19):
        ...
        def _fori__20(j, _acc__21):
            ...
            def _fori__22(i, _acc__23):
                ...
                accumulator = accumulator.at[i - 1].set(logbeta[t - 1, i - 
                    1] + log_real(A[j - 1, i - 1]) + normal_lpdf(y[t - 1],
                    mu[i - 1], sigma[i - 1]))
                return accumulator
            accumulator = lax_fori_loop(1, K + 1, _fori__22, accumulator)
            logbeta = logbeta.at[t - 1 - 1, j - 1].set(log_sum_exp_array(
                accumulator))
    ...
    def _fori__30(j, _acc__31):
        delta = _acc__31
        delta = delta.at[1 - 1, K - 1].set(normal_lpdf(y[1 - 1], mu[j - 1],
            sigma[j - 1]))
        return delta
    ...
    def _fori__32(t, _acc__33):
        ...
        def _fori__34(j, _acc__35):
            ...
            def _fori__36(i, _acc__37):
                ...
                logp = delta[t - 1 - 1, i - 1] + log_real(A[i - 1, j - 1]
                    ) + normal_lpdf(y[t - 1], mu[j - 1], sigma[j - 1])
                ...
                bpointer, delta = lax_cond(logp > delta[t - 1, j - 1],
                    _then__38, _else__40, (bpointer, delta))
                return bpointer, delta
            bpointer, delta = lax_fori_loop(1, K + 1, _fori__36, (bpointer,
                delta))
            return bpointer, delta
        bpointer, delta = lax_fori_loop(1, K + 1, _fori__34, (bpointer, delta))
        return bpointer, delta
    ...
    def _fori__41(j, _acc__42):
        ...
        def _else__45(acc):
            return acc
        zstar = lax_cond(delta[T - 1, j - 1] == logp_zstar, _then__43,
            _else__45, zstar)
        return zstar
    ...
    return {'logalpha': logalpha, 'accumulator': accumulator, 'alpha':
        alpha, 'logbeta': logbeta, 'accumulator': accumulator, 'beta':
        beta__, 'loggamma': loggamma, 'gamma': gamma__, 'delta': delta,
        'bpointer': bpointer, 'logp_zstar': logp_zstar, 'zstar': zstar}
...
def map_generated_quantities(_samples, *, K, T, y):
    ...
    return _f(_samples['pi1'], _samples['A'], _samples['mu'], _samples['sigma']
        )


def parameters_info(*, K, T, y):
    return {'pi1': {'shape': [K]}, 'A': {'shape': [K, K]}, 'mu': {'shape':
        [K]}, 'sigma': {'shape': [K]}}
