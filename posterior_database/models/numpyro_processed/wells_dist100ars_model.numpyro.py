...
def transformed_data(*, N, switched, dist, arsenic):
    ...
    x = transpose(array([dist100, arsenic], dtype=dtype_float), 0, 1)
    return {'dist100': dist100, 'x': x}


def model(*, N, switched, dist, arsenic, dist100, x):
...
def parameters_info(*, N, switched, dist, arsenic, dist100, x):
    return {'alpha': {'shape': []}, 'beta': {'shape': [2]}}
