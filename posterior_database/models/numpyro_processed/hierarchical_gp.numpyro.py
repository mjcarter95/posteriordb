...
def convert_inputs(inputs):
    ...
    return {'N_regions': N_regions, 'N_years_obs': N_years_obs, 'N_years':
        N_years, 'N_states': N_states, 'state_region_ind': state_region_ind,
        'N': N, 'state_ind': state_ind, 'region_ind': region_ind,
        'year_ind': year_ind, 'y': y}


def transformed_data(*, N_regions, N_years_obs, N_years, N_states,
    state_region_ind, N, state_ind, region_ind, year_ind, y):
    years = empty([N_years], dtype=dtype_float)

    @jit
    def _fori__1(t, _acc__2):
        years = _acc__2
        years = years.at[t - 1].set(t)
        return years
...
def model(*, N_regions, N_years_obs, N_years, N_states, state_region_ind, N,
    ...
    prop_var = sample('prop_var', simplex_constrained_improper_uniform(
        shape=[17]))
    mu = sample('mu', improper_uniform(shape=[]))
    length_GP_region_long = sample('length_GP_region_long',
        lower_constrained_improper_uniform(0, shape=[]))
    length_GP_state_long = sample('length_GP_state_long',
        lower_constrained_improper_uniform(0, shape=[]))
    length_GP_region_short = sample('length_GP_region_short',
        lower_constrained_improper_uniform(0, shape=[]))
    length_GP_state_short = sample('length_GP_state_short',
        lower_constrained_improper_uniform(0, shape=[]))
    ...
    def _fori__5(i, _acc__6):
        sigma_state = _acc__6
        sigma_state = sigma_state.at[i - 1].set(sqrt_real(vars[i + 2 - 1]))
        return sigma_state
    ...
    cov_region = gp_exp_quad_cov_array_real_real(years,
        sigma_GP_region_long, length_GP_region_long
        ) + gp_exp_quad_cov_array_real_real(years, sigma_GP_region_short,
        length_GP_region_short)
    cov_state = gp_exp_quad_cov_array_real_real(years, sigma_GP_state_long,
        length_GP_state_long) + gp_exp_quad_cov_array_real_real(years,
        sigma_GP_state_short, length_GP_state_short)

    @jit
    def _fori__7(year, _acc__8):
        cov_region, cov_state = _acc__8
        cov_region = cov_region.at[year - 1, year - 1].set(cov_region[year -
            1, year - 1] + array(1e-06, dtype=dtype_float))
        cov_state = cov_state.at[year - 1, year - 1].set(cov_state[year - 1,
            year - 1] + array(1e-06, dtype=dtype_float))
    ...
    def _fori__9(n, _acc__10):
        obs_mu = _acc__10
        obs_mu = obs_mu.at[n - 1].set(mu + year_re[year_ind[n - 1] - 1] +
            state_re[state_ind[n - 1] - 1] + region_re[region_ind[n - 1] - 
            1] + GP_region[year_ind[n - 1] - 1, region_ind[n - 1] - 1] +
            GP_state[year_ind[n - 1] - 1, state_ind[n - 1] - 1])
        return obs_mu
    ...
    observe('_mu__17', normal(array(0.5, dtype=dtype_float), array(0.5,
    ...
    observe('_length_GP_region_short__22', weibull(30, 3),
        length_GP_region_short)
...
def generated_quantities(*, N_regions, N_years_obs, N_years, N_states,
    state_region_ind, N, state_ind, region_ind, year_ind, y, years, counts,
    GP_region_std, GP_state_std, year_std, state_std, region_std, tot_var,
    prop_var, mu, length_GP_region_long, length_GP_state_long,
    ...
    def _fori__28(state, _acc__29):
        ...
        def _fori__30(t, _acc__31):
            ...
            def _then__32(_acc__33):
                y_new = _acc__33
                y_new = y_new.at[t - 1, state - 1].set(state_re[state - 1] +
                    region_re[state_region_ind[state - 1] - 1] + GP_state[t -
                    1, state - 1] + GP_region[t - 1, state_region_ind[state -
                    1] - 1] + (mu + year_re[t - 1]))
                return y_new
            ...
            y_new_pred = y_new_pred.at[t - 1, state - 1].set(normal_rng(
                y_new[t - 1, state - 1], sigma_error_state_2))
            return y_new, y_new_pred
        y_new, y_new_pred = lax_fori_loop(1, N_years + 1, _fori__30, (y_new,
            y_new_pred))
        return y_new, y_new_pred
    y_new, y_new_pred = lax_fori_loop(1, N_states + 1, _fori__28, (y_new,
        y_new_pred))
    return {'vars': vars, 'sigma_year': sigma_year, 'sigma_region':
        sigma_region, 'sigma_state': sigma_state, 'sigma_GP_region_long':
        sigma_GP_region_long, 'sigma_GP_state_long': sigma_GP_state_long,
        'sigma_GP_region_short': sigma_GP_region_short,
        'sigma_GP_state_short': sigma_GP_state_short, 'sigma_error_state_2':
        sigma_error_state_2, 'region_re': region_re, 'year_re': year_re,
        'state_re': state_re, 'cov_region': cov_region, 'cov_state':
        cov_state, 'L_cov_region': L_cov_region, 'L_cov_state': L_cov_state,
        'GP_region': GP_region, 'GP_state': GP_state, 'level': level,
        'y_new': y_new, 'y_new_pred': y_new_pred}


def map_generated_quantities(_samples, *, N_regions, N_years_obs, N_years,
    N_states, state_region_ind, N, state_ind, region_ind, year_ind, y,
    years, counts):

    def _generated_quantities(GP_region_std, GP_state_std, year_std,
        state_std, region_std, tot_var, prop_var, mu, length_GP_region_long,
        length_GP_state_long, length_GP_region_short, length_GP_state_short):
        return generated_quantities(N_regions=N_regions, N_years_obs=
            N_years_obs, N_years=N_years, N_states=N_states,
            state_region_ind=state_region_ind, N=N, state_ind=state_ind,
            region_ind=region_ind, year_ind=year_ind, y=y, years=years,
            counts=counts, GP_region_std=GP_region_std, GP_state_std=
            GP_state_std, year_std=year_std, state_std=state_std,
            region_std=region_std, tot_var=tot_var, prop_var=prop_var, mu=
            mu, length_GP_region_long=length_GP_region_long,
    ...
    return _f(_samples['GP_region_std'], _samples['GP_state_std'], _samples
        ['year_std'], _samples['state_std'], _samples['region_std'],
        _samples['tot_var'], _samples['prop_var'], _samples['mu'], _samples
        ['length_GP_region_long'], _samples['length_GP_state_long'],
        _samples['length_GP_region_short'], _samples['length_GP_state_short'])


def parameters_info(*, N_regions, N_years_obs, N_years, N_states,
    state_region_ind, N, state_ind, region_ind, year_ind, y, years, counts):
    return {'GP_region_std': {'shape': [N_years, N_regions]},
        'GP_state_std': {'shape': [N_years, N_states]}, 'year_std': {
        'shape': [N_years_obs]}, 'state_std': {'shape': [N_states]},
        'region_std': {'shape': [N_regions]}, 'tot_var': {'shape': []},
        'prop_var': {'shape': [17]}, 'mu': {'shape': []},
        'length_GP_region_long': {'shape': []}, 'length_GP_state_long': {
        'shape': []}, 'length_GP_region_short': {'shape': []},
