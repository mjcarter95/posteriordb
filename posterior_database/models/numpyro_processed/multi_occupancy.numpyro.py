...
def cov_matrix_2d(sigma, rho):
    ...
    Sigma = Sigma.at[1 - 1, 1 - 1].set(square_real(sigma[1 - 1]))
    Sigma = Sigma.at[2 - 1, 2 - 1].set(square_real(sigma[2 - 1]))
    Sigma = Sigma.at[1 - 1, 2 - 1].set(sigma[1 - 1] * sigma[2 - 1] * rho)
    Sigma = Sigma.at[2 - 1, 1 - 1].set(Sigma[1 - 1, 2 - 1])
    return Sigma
...
def model(*, J, K, n, X, S):
    ...
    sigma_uv = sample('sigma_uv', lower_constrained_improper_uniform(0,
        shape=[2]))
    ...
    def _fori__1(i, _acc__2):
        ...
        uv = uv.at[i - 1, 1 - 1].set(uv1[i - 1])
        uv = uv.at[i - 1, 2 - 1].set(uv2[i - 1])
        logit_psi = logit_psi.at[i - 1].set(uv[i - 1, 1 - 1] + alpha)
        return logit_psi, uv
    ...
    def _fori__3(i, _acc__4):
        logit_theta = _acc__4
        logit_theta = logit_theta.at[i - 1].set(uv[i - 1, 2 - 1] + beta__)
        return logit_theta
        ...
        def _fori__14(j, _acc__15):
            ...
            def _then__16(_acc__17):
                factor(f'_expr__{j}__{i}__20', lp_observed(X[i - 1, j - 1],
                    K, logit_psi[i - 1], logit_theta[i - 1]))
                return None

            def _else__18(_acc__19):
                factor(f'_expr__{j}__{i}__21', lp_unobserved(K, logit_psi[i -
                    1], logit_theta[i - 1]))
                return None
...
def generated_quantities(*, J, K, n, X, S, alpha, beta__, Omega, rho_uv,
    ...
    sim_uv = multi_normal_rng(rep_vector_int_int(0, 2), cov_matrix_2d(
        sigma_uv, rho_uv))
    logit_psi_sim = alpha + sim_uv[1 - 1]
    logit_theta_sim = beta__ + sim_uv[2 - 1]
    return {'uv': uv, 'logit_psi': logit_psi, 'logit_theta': logit_theta,
        'E_N': E_N, 'E_N_2': E_N_2, 'sim_uv': sim_uv, 'logit_psi_sim':
        logit_psi_sim, 'logit_theta_sim': logit_theta_sim}


def map_generated_quantities(_samples, *, J, K, n, X, S):

    def _generated_quantities(alpha, beta__, Omega, rho_uv, sigma_uv, uv1, uv2
        ):
        return generated_quantities(J=J, K=K, n=n, X=X, S=S, alpha=alpha,
            beta__=beta__, Omega=Omega, rho_uv=rho_uv, sigma_uv=sigma_uv,
            uv1=uv1, uv2=uv2)
    _f = jit(vmap(_generated_quantities))
    return _f(_samples['alpha'], _samples['beta'], _samples['Omega'],
        _samples['rho_uv'], _samples['sigma_uv'], _samples['uv1'], _samples
        ['uv2'])


def parameters_info(*, J, K, n, X, S):
    return {'alpha': {'shape': []}, 'beta': {'shape': []}, 'Omega': {
        'shape': []}, 'rho_uv': {'shape': []}, 'sigma_uv': {'shape': [2]},
        'uv1': {'shape': [S]}, 'uv2': {'shape': [S]}}
