...
def convert_inputs(inputs):
    ...
    rho = array(inputs['rho'], dtype=dtype_float)
    return {'N': N, 'u': u, 'v': v, 'K': K, 'alpha': alpha, 'tau': tau,
        'rho': rho}


def model(*, N, u, v, K, alpha, tau, rho):
    ...
    phi = sample('phi', ordered_constrained_improper_uniform(shape=[K]))
    lambda__ = sample('lambda', ordered_constrained_improper_uniform(shape=[K])
        )
    ...
    def _fori__8(k, _acc__9):
        gamma__ = _acc__9
        gamma__ = gamma__.at[1 - 1, k - 1].set(normal_lpdf(u[1 - 1], phi[k -
            1], tau) + normal_lpdf(v[1 - 1], lambda__[k - 1], rho))
        return gamma__
        ...
        def _fori__12(k, _acc__13):
            ...
            def _fori__14(j, _acc__15):
                ...
                acc = acc.at[j - 1].set(gamma__[t - 1 - 1, j - 1] +
                    log_real(theta[j - 1, k - 1]) + normal_lpdf(u[t - 1],
                    phi[k - 1], tau) + normal_lpdf(v[t - 1], lambda__[k - 1
                    ], rho))
                return acc
...
def generated_quantities(*, N, u, v, K, alpha, tau, rho, theta1, theta2,
    phi, lambda__):
    theta = empty([K, K], dtype=dtype_float)
    theta = theta.at[1 - 1].set(theta1)
    theta = theta.at[2 - 1].set(theta2)
    ...
    def _fori__17(k, _acc__18):
        best_logp = _acc__18
        best_logp = best_logp.at[1 - 1, K - 1].set(normal_lpdf(u[1 - 1],
            phi[k - 1], tau) + normal_lpdf(v[1 - 1], lambda__[k - 1], rho))
        return best_logp
        ...
        def _fori__21(k, _acc__22):
            ...
            def _fori__23(j, _acc__24):
                ...
                logp = best_logp[t - 1 - 1, j - 1] + log_real(theta[j - 1, 
                    k - 1]) + normal_lpdf(u[t - 1], phi[k - 1], tau
                    ) + normal_lpdf(v[t - 1], lambda__[k - 1], rho)
...
def map_generated_quantities(_samples, *, N, u, v, K, alpha, tau, rho):

    def _generated_quantities(theta1, theta2, phi, lambda__):
        return generated_quantities(N=N, u=u, v=v, K=K, alpha=alpha, tau=
            tau, rho=rho, theta1=theta1, theta2=theta2, phi=phi, lambda__=
            lambda__)
...
def parameters_info(*, N, u, v, K, alpha, tau, rho):
