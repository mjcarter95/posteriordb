...
def model(*, N, ts, y_init, y):
    theta = sample('theta', lower_constrained_improper_uniform(0, shape=[4]))
    z_init = sample('z_init', lower_constrained_improper_uniform(0, shape=[2]))
    sigma = sample('sigma', lower_constrained_improper_uniform(0, shape=[2]))
    z = (
        integrate_ode_rk45_function_array_int_array_array_array_array_real_real_real
        (dz_dt, z_init, 0, ts, theta, rep_array_real_int(array(0.0, dtype=
        dtype_float), 0), rep_array_int_int(0, 0), array(1e-05, dtype=
        dtype_float), array(0.001, dtype=dtype_float), array(500.0, dtype=
        dtype_float)))
    observe('_theta__1', normal(1, array(0.5, dtype=dtype_float)), theta[
        array([1, 3], dtype=dtype_long) - 1])
    observe('_theta__2', normal(array(0.05, dtype=dtype_float), array(0.05,
        dtype=dtype_float)), theta[array([2, 4], dtype=dtype_long) - 1])
    ...
    def _fori__5(k, _acc__6):
        ...
        observe(f'_y__{k}__8', lognormal(log_array(z[:, k - 1]), sigma[k - 
            1]), y[:, k - 1])
        return None
...
def generated_quantities(*, N, ts, y_init, y, theta, z_init, sigma):
    ...
    def _fori__9(k, _acc__10):
        ...
        y_init_rep = y_init_rep.at[k - 1].set(lognormal_rng(log_real(z_init
            [k - 1]), sigma[k - 1]))

        @jit
        def _fori__11(n, _acc__12):
            y_rep = _acc__12
            y_rep = y_rep.at[n - 1, k - 1].set(lognormal_rng(log_real(z[n -
                1, k - 1]), sigma[k - 1]))
            return y_rep
        y_rep = lax_fori_loop(1, N + 1, _fori__11, y_rep)
        return y_init_rep, y_rep
    y_init_rep, y_rep = lax_fori_loop(1, 2 + 1, _fori__9, (y_init_rep, y_rep))
...
def map_generated_quantities(_samples, *, N, ts, y_init, y):
    ...
    def _generated_quantities(theta, z_init, sigma):
        return generated_quantities(N=N, ts=ts, y_init=y_init, y=y, theta=
            theta, z_init=z_init, sigma=sigma)
...
def parameters_info(*, N, ts, y_init, y):
    return {'theta': {'shape': [4]}, 'z_init': {'shape': [2]}, 'sigma': {
        'shape': [2]}}
