...
def model(*, N, y):
    ...
    def _fori__5(n, _acc__6):
        factor(f'_expr__{n}__7', log_mix_real_real_real(theta, normal_lpdf(
            y[n - 1], mu[1 - 1], array(1.0, dtype=dtype_float)),
            normal_lpdf(y[n - 1], mu[2 - 1], array(1.0, dtype=dtype_float))))
        return None
