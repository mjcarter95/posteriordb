...
def model(*, N, Npts, rat, x, y, xbar):
    ...
    def _fori__5(n, _acc__6):
        irat = rat[n - 1]
        observe(f'_y__{n}__7', normal(alpha[irat - 1] + beta__[irat - 1] *
            (x[n - 1] - xbar), sigma_y), y[n - 1])
        return None
    _ = fori_loop(1, Npts + 1, _fori__5, None)


def generated_quantities(*, N, Npts, rat, x, y, xbar, alpha, beta__,
    mu_alpha, mu_beta, sigma_y, sigma_alpha, sigma_beta):
...
def map_generated_quantities(_samples, *, N, Npts, rat, x, y, xbar):
    ...
    def _generated_quantities(alpha, beta__, mu_alpha, mu_beta, sigma_y,
        ...
        return generated_quantities(N=N, Npts=Npts, rat=rat, x=x, y=y, xbar
            =xbar, alpha=alpha, beta__=beta__, mu_alpha=mu_alpha, mu_beta=
            mu_beta, sigma_y=sigma_y, sigma_alpha=sigma_alpha, sigma_beta=
            sigma_beta)
    _f = jit(vmap(_generated_quantities))
    return _f(_samples['alpha'], _samples['beta'], _samples['mu_alpha'],
        _samples['mu_beta'], _samples['sigma_y'], _samples['sigma_alpha'],
        _samples['sigma_beta'])


def parameters_info(*, N, Npts, rat, x, y, xbar):
    return {'alpha': {'shape': [N]}, 'beta': {'shape': [N]}, 'mu_alpha': {
        'shape': []}, 'mu_beta': {'shape': []}, 'sigma_y': {'shape': []},
        'sigma_alpha': {'shape': []}, 'sigma_beta': {'shape': []}}
