...
def rsm(y, theta, beta__, kappa):
    unsummed = append_row_vector_vector(rep_vector_int_int(0, 1), theta -
        beta__ - kappa)
...
def model(*, I, N, ii, J, jj, y, K, W, m, adj, W_adj):
    ...
    beta__ = append_row_vector_vector(beta_free, rep_vector_real_int(-1 *
        sum_vector(beta_free), 1))
    kappa = append_row_vector_vector(kappa_free, rep_vector_real_int(-1 *
        sum_vector(kappa_free), 1))
    ...
    def _fori__21(n, _acc__22):
        factor(f'_expr__{n}__23', rsm(y[n - 1], theta[jj[n - 1] - 1] *
            alpha[ii[n - 1] - 1], beta__[ii[n - 1] - 1], kappa))
        return None
...
def map_generated_quantities(_samples, *, I, N, ii, J, jj, y, K, W, m, adj,
    ...
    def _generated_quantities(alpha, beta_free, kappa_free, theta, lambda_adj):
        return generated_quantities(I=I, N=N, ii=ii, J=J, jj=jj, y=y, K=K,
            W=W, m=m, adj=adj, W_adj=W_adj, alpha=alpha, beta_free=
            beta_free, kappa_free=kappa_free, theta=theta, lambda_adj=
            lambda_adj)
    _f = jit(vmap(_generated_quantities))
    return _f(_samples['alpha'], _samples['beta_free'], _samples[
        'kappa_free'], _samples['theta'], _samples['lambda_adj'])


def parameters_info(*, I, N, ii, J, jj, y, K, W, m, adj, W_adj):
    return {'alpha': {'shape': [I]}, 'beta_free': {'shape': [I - 1]},
        'kappa_free': {'shape': [m - 1]}, 'theta': {'shape': [J]},
        'lambda_adj': {'shape': [K]}}
