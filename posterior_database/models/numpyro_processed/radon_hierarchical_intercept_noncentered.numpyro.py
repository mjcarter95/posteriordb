...
def model(*, J, N, county_idx, log_uppm, floor_measure, log_radon):
    ...
    def _fori__6(n, _acc__7):
        mu, muj = _acc__7
        muj = muj.at[n - 1].set(alpha[county_idx[n - 1] - 1] + log_uppm[n -
            1] * beta__[1 - 1])
        mu = mu.at[n - 1].set(muj[n - 1] + floor_measure[n - 1] * beta__[2 - 1]
            )
...
def generated_quantities(*, J, N, county_idx, log_uppm, floor_measure,
    log_radon, alpha_raw, beta__, mu_alpha, sigma_alpha, sigma_y):
    alpha = mu_alpha + sigma_alpha * alpha_raw
    return {'alpha': alpha}
...
def map_generated_quantities(_samples, *, J, N, county_idx, log_uppm,
    ...
    def _generated_quantities(alpha_raw, beta__, mu_alpha, sigma_alpha, sigma_y
        ):
        return generated_quantities(J=J, N=N, county_idx=county_idx,
            log_uppm=log_uppm, floor_measure=floor_measure, log_radon=
            log_radon, alpha_raw=alpha_raw, beta__=beta__, mu_alpha=
            mu_alpha, sigma_alpha=sigma_alpha, sigma_y=sigma_y)
...
def parameters_info(*, J, N, county_idx, log_uppm, floor_measure, log_radon):
    return {'alpha_raw': {'shape': [J]}, 'beta': {'shape': [2]}, 'mu_alpha':
        {'shape': []}, 'sigma_alpha': {'shape': []}, 'sigma_y': {'shape': []}}
