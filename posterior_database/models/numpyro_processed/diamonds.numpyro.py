...
def transformed_data(*, N, Y, K, X, prior_only):
    ...
    def _fori__1(i, _acc__2):
        Xc, means_X = _acc__2
        means_X = means_X.at[i - 1 - 1].set(mean_vector(X[:, i - 1]))
        Xc = Xc.at[:, i - 1 - 1].set(X[:, i - 1] - means_X[i - 1 - 1])
        return Xc, means_X
...
def model(*, N, Y, K, X, prior_only, Kc, Xc, means_X):
    ...
    factor('_expr__5', student_t_lpdf(sigma, 3, 0, 10) - 1 *
        student_t_lccdf(0, 3, 0, 10))
...
def map_generated_quantities(_samples, *, N, Y, K, X, prior_only, Kc, Xc,
    ...
    def _generated_quantities(b, Intercept, sigma):
        return generated_quantities(N=N, Y=Y, K=K, X=X, prior_only=
            prior_only, Kc=Kc, Xc=Xc, means_X=means_X, b=b, Intercept=
            Intercept, sigma=sigma)
...
def parameters_info(*, N, Y, K, X, prior_only, Kc, Xc, means_X):
    return {'b': {'shape': [Kc]}, 'Intercept': {'shape': []}, 'sigma': {
        'shape': []}}
