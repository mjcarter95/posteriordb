...
def model(*, I, N, ii, J, jj, y, K, W, adj, W_adj):
    ...
    beta__ = beta__.at[1 - 1:I - 1].set(beta_free)
    beta__ = beta__.at[I - 1].set(-1 * sum_vector(beta_free))
    ...
    observe('_y__20', bernoulli_logit(alpha[ii - 1] * theta[jj - 1] -
        beta__[ii - 1]), y)
...
def map_generated_quantities(_samples, *, I, N, ii, J, jj, y, K, W, adj, W_adj
    ):

    def _generated_quantities(alpha, beta_free, theta, lambda_adj):
        return generated_quantities(I=I, N=N, ii=ii, J=J, jj=jj, y=y, K=K,
            W=W, adj=adj, W_adj=W_adj, alpha=alpha, beta_free=beta_free,
