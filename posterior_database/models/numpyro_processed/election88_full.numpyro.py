...
def convert_inputs(inputs):
    ...
    return {'n_age': n_age, 'N': N, 'age': age, 'n_age_edu': n_age_edu,
        ...
        'female': female, 'n_region_full': n_region_full, 'region_full':
        region_full, 'n_state': n_state, 'state': state, 'v_prev_full':
        v_prev_full, 'y': y}
...
def model(*, n_age, N, age, n_age_edu, age_edu, black, n_edu, edu, female,
    ...
    def _fori__1(i, _acc__2):
        y_hat = _acc__2
        y_hat = y_hat.at[i - 1].set(beta__[1 - 1] + beta__[2 - 1] * black[i -
            1] + beta__[3 - 1] * female[i - 1] + beta__[5 - 1] * female[i -
            1] * black[i - 1] + beta__[4 - 1] * v_prev_full[i - 1] + a[age[
            i - 1] - 1] + b[edu[i - 1] - 1] + c[age_edu[i - 1] - 1] + d[
...
def generated_quantities(*, n_age, N, age, n_age_edu, age_edu, black, n_edu,
    edu, female, n_region_full, region_full, n_state, state, v_prev_full, y,
    a, b, c, d, e__, beta__, sigma_a, sigma_b, sigma_c, sigma_d, sigma_e):
    y_hat = empty([N], dtype=dtype_float)
...
def map_generated_quantities(_samples, *, n_age, N, age, n_age_edu, age_edu,
    black, n_edu, edu, female, n_region_full, region_full, n_state, state,
    v_prev_full, y):

    def _generated_quantities(a, b, c, d, e__, beta__, sigma_a, sigma_b,
        sigma_c, sigma_d, sigma_e):
        return generated_quantities(n_age=n_age, N=N, age=age, n_age_edu=
            n_age_edu, age_edu=age_edu, black=black, n_edu=n_edu, edu=edu,
            female=female, n_region_full=n_region_full, region_full=
            region_full, n_state=n_state, state=state, v_prev_full=
            v_prev_full, y=y, a=a, b=b, c=c, d=d, e__=e__, beta__=beta__,
            sigma_a=sigma_a, sigma_b=sigma_b, sigma_c=sigma_c, sigma_d=
            sigma_d, sigma_e=sigma_e)
    _f = jit(vmap(_generated_quantities))
    return _f(_samples['a'], _samples['b'], _samples['c'], _samples['d'],
        _samples['e'], _samples['beta'], _samples['sigma_a'], _samples[
        'sigma_b'], _samples['sigma_c'], _samples['sigma_d'], _samples[
        'sigma_e'])


def parameters_info(*, n_age, N, age, n_age_edu, age_edu, black, n_edu, edu,
    female, n_region_full, region_full, n_state, state, v_prev_full, y):
    return {'a': {'shape': [n_age]}, 'b': {'shape': [n_edu]}, 'c': {'shape':
        [n_age_edu]}, 'd': {'shape': [n_state]}, 'e': {'shape': [
        n_region_full]}, 'beta': {'shape': [5]}, 'sigma_a': {'shape': []},
        'sigma_b': {'shape': []}, 'sigma_c': {'shape': []}, 'sigma_d': {
        'shape': []}, 'sigma_e': {'shape': []}}
