...
def model(*, K, N, y):
    theta = sample('theta', simplex_constrained_improper_uniform(shape=[K]))
    ...
    def _fori__2(n, _acc__3):
        ...
        def _fori__4(k, _acc__5):
            ps = _acc__5
            ps = ps.at[k - 1].set(log_real(theta[k - 1]) + normal_lpdf(y[n -
                1], mu[k - 1], sigma[k - 1]))
            return ps
...
def parameters_info(*, K, N, y):
    return {'theta': {'shape': [K]}, 'mu': {'shape': [K]}, 'sigma': {
        'shape': [K]}}
