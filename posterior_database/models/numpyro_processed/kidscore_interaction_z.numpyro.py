...
def transformed_data(*, N, kid_score, mom_hs, mom_iq):
    z_mom_hs = true_divide(mom_hs - mean_vector(mom_hs), 2 * sd_vector(mom_hs))
    z_mom_iq = true_divide(mom_iq - mean_vector(mom_iq), 2 * sd_vector(mom_iq))
...
def model(*, N, kid_score, mom_hs, mom_iq, z_mom_hs, z_mom_iq, inter):
    ...
    observe('_kid_score__1', normal(beta__[1 - 1] + beta__[2 - 1] *
        z_mom_hs + beta__[3 - 1] * z_mom_iq + beta__[4 - 1] * inter, sigma),
        kid_score)


def parameters_info(*, N, kid_score, mom_hs, mom_iq, z_mom_hs, z_mom_iq, inter
    ):
