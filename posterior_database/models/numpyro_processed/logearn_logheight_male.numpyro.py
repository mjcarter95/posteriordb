...
def transformed_data(*, N, earn, height, male):
    ...
    log_height = log_vector(height)
    return {'log_earn': log_earn, 'log_height': log_height}


def model(*, N, earn, height, male, log_earn, log_height):
    beta__ = sample('beta', improper_uniform(shape=[3]))
    sigma = sample('sigma', lower_constrained_improper_uniform(0, shape=[]))
    observe('_log_earn__1', normal(beta__[1 - 1] + beta__[2 - 1] *
        log_height + beta__[3 - 1] * male, sigma), log_earn)


def parameters_info(*, N, earn, height, male, log_earn, log_height):
