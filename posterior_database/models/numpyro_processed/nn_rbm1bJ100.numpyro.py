...
def transformed_data(*, M, N, x, K, y):
    ...
    s2_0_alpha = true_divide(array(0.05, dtype=dtype_float), M **
        true_divide(1, nu_alpha)) ** 2
    nu_beta = array(0.5, dtype=dtype_float)
    s2_0_beta = true_divide(array(0.05, dtype=dtype_float), J **
        true_divide(1, nu_beta)) ** 2
    ones__ = rep_vector_int_int(1, N)
    x1 = append_col_vector_matrix(ones__, x)
    return {'J': J, 'nu_alpha': nu_alpha, 's2_0_alpha': s2_0_alpha,
        'nu_beta': nu_beta, 's2_0_beta': s2_0_beta, 'ones__': ones__, 'x1': x1}


def model(*, M, N, x, K, y, J, nu_alpha, s2_0_alpha, nu_beta, s2_0_beta,
    ones__, x1):
    sigma2_alpha = sample('sigma2_alpha',
        lower_constrained_improper_uniform(0, shape=[]))
    sigma2_beta = sample('sigma2_beta', lower_constrained_improper_uniform(
        0, shape=[]))
    ...
    v = append_col_vector_matrix(ones__, matmul(append_col_vector_matrix(
        ones__, tanh_matrix(matmul(x1, append_row_rowvector_matrix(alpha1,
        alpha)))), append_row_rowvector_matrix(beta1, beta__)))
    ...
    observe('_expr__5', normal(0, sqrt_real(sigma2_alpha)),
        to_vector_matrix(alpha))
...
def parameters_info(*, M, N, x, K, y, J, nu_alpha, s2_0_alpha, nu_beta,
    ...
    return {'sigma2_alpha': {'shape': []}, 'sigma2_beta': {'shape': []},
        'alpha': {'shape': [M, J]}, 'beta': {'shape': [J, K - 1]}, 'alpha1':
        {'shape': [J]}, 'beta1': {'shape': [K - 1]}}
