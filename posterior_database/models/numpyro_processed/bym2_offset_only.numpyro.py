...
def convert_inputs(inputs):
    ...
    return {'N_edges': N_edges, 'N': N, 'node1': node1, 'node2': node2, 'y':
        y, 'E': E, 'scaling_factor': scaling_factor}
...
def model(*, N_edges, N, node1, node2, y, E, scaling_factor, log_E):
    ...
    convolved_re = sqrt_real(1 - rho) * theta + sqrt_real(true_divide(rho,
        scaling_factor)) * phi
    observe('_y__1', poisson_log(log_E + beta0 + convolved_re * sigma), y)
    factor('_expr__2', -array(0.5, dtype=dtype_float) * dot_self_vector(phi
        [node1 - 1] - phi[node2 - 1]))
    observe('_beta0__3', normal(0, 1), beta0)
    observe('_theta__4', normal(0, 1), theta)
    observe('_sigma__5', normal(0, 1), sigma)
    observe('_rho__6', beta(array(0.5, dtype=dtype_float), array(0.5, dtype
        =dtype_float)), rho)
    observe('_expr__7', normal(0, array(0.001, dtype=dtype_float) * N),
        sum_vector(phi))
...
def map_generated_quantities(_samples, *, N_edges, N, node1, node2, y, E,
    ...
    def _generated_quantities(beta0, sigma, rho, theta, phi):
        return generated_quantities(N_edges=N_edges, N=N, node1=node1,
            node2=node2, y=y, E=E, scaling_factor=scaling_factor, log_E=
            log_E, beta0=beta0, sigma=sigma, rho=rho, theta=theta, phi=phi)
...
def parameters_info(*, N_edges, N, node1, node2, y, E, scaling_factor, log_E):
    return {'beta0': {'shape': []}, 'sigma': {'shape': []}, 'rho': {'shape':
        []}, 'theta': {'shape': [N]}, 'phi': {'shape': [N]}}
