...
def model(*, n, y, x, w):
    mu = sample('mu', uniform(mean_vector(y) - 3 * sd_vector(y) * ones([n]),
        mean_vector(y) + 3 * sd_vector(y)))
    ...
    sigma = sample('sigma', positive_ordered_constrained_improper_uniform(
        shape=[3]))
    yhat = mu + beta__ * x + lambda__ * w

    def _fori__1(t, _acc__2):
        observe(f'_seasonal__{t}__3', normal(-sum_vector(seasonal[t - 11 - 
            1:t - 1]), sigma[1 - 1]), seasonal[t - 1])
        return None
    _ = fori_loop(12, n + 1, _fori__1, None)

    def _fori__4(t, _acc__5):
        observe(f'_mu__{t}__6', normal(mu[t - 1 - 1], sigma[2 - 1]), mu[t - 1])
        return None
...
def map_generated_quantities(_samples, *, n, y, x, w):
    ...
    def _generated_quantities(mu, seasonal, beta__, lambda__, sigma):
        return generated_quantities(n=n, y=y, x=x, w=w, mu=mu, seasonal=
            seasonal, beta__=beta__, lambda__=lambda__, sigma=sigma)
...
def parameters_info(*, n, y, x, w):
    return {'mu': {'shape': [n]}, 'seasonal': {'shape': [n]}, 'beta': {
        'shape': []}, 'lambda': {'shape': []}, 'sigma': {'shape': [3]}}
