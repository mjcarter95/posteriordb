...
def transformed_data(*, N, switched, dist):
    dist100 = true_divide(dist, array(100.0, dtype=dtype_float))
    x = transpose(array([dist100], dtype=dtype_float), 0, 1)
    return {'dist100': dist100, 'x': x}


def model(*, N, switched, dist, dist100, x):
...
def parameters_info(*, N, switched, dist, dist100, x):
