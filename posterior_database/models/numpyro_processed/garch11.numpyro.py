...
def model(*, T, y, sigma1):
    ...
    sigma = sigma.at[1 - 1].set(sigma1)

    @jit
    def _fori__1(t, _acc__2):
        sigma = _acc__2
        sigma = sigma.at[t - 1].set(sqrt_real(alpha0 + alpha1 * square_real
            (y[t - 1 - 1] - mu) + beta1 * square_real(sigma[t - 1 - 1])))
        return sigma
...
def parameters_info(*, T, y, sigma1):
    return {'mu': {'shape': []}, 'alpha0': {'shape': []}, 'alpha1': {
        'shape': []}, 'beta1': {'shape': []}}
